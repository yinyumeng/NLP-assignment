...
public class LogisticRegression {
    ...
    private int maxInterations= 5000;
    private double thresholdDelta = 0.003 ;
...
//	        System.out.println(FeatureList.subList(2, FeatureList.size()).toString().replaceAll("[^a-zA-Z, ]",""));
	        bw.write(FeatureList.toString().replaceAll("[^a-zA-Z, ]",""));
...
//	                System.out.println("the label is "+label+"the predicted is "+predicted);
				...
				if(Math.sqrt(sumDelta) <thresholdDelta) {
					System.out.println("STOP criteria met: Iteration " + n + ", Log-likelihood = " + lik+" sqrt of sumdelta " + Math.sqrt(sumDelta));
					break;
				}
...
//            
//	        System.out.println("size is: " +  (FeatureList.size()));
    ...
    public void test(List<List<Integer>> InputData, Vector<String> FeatureList){
    	try{
		    ...
		    if((line = br.readLine())!=null){
                ...
                System.out.println("size is: " + weightList.length);
		    }
            System.out.println("size is: " + weights.length);
